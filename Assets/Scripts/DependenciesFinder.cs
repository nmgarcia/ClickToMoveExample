...
/// </summary>
public class DependenciesFinder : EditorWindow
{
    ...
    private void PopulateUnusedAssetsList()
    {
        ...
        foreach (var mapElement in map)
        {
            ...
            if (validForOutput && ExistFile(mapElement.Key))
            {
    ...
    private bool ExistFile(string key)
    {
        bool exists = File.Exists(@"" + key);
    ...
    private void OnGUI()
    {
        ...
        if (filteredAssets.Count < 1000)
        {
            if (GUILayout.Button("Save to Clipboard", GUILayout.Width(250f)))
            {
                ...
                {
                    toClipboard.AppendLine(asset.Path);
                }

                EditorGUIUtility.systemCopyBuffer = toClipboard.ToString();
            }

            if (GUILayout.Button("Delete Unreferenced", GUILayout.Width(250f)))
            {
                
                foreach (var asset in filteredAssets)
                {    
                    DeleteUnreferenced(asset);
                }
    ...
    private void DeleteUnreferenced(AssetData asset)
    {        
        if (ExistFile(asset.Path))
        {
            File.Delete(@"" + asset.Path);
        }

        if (ExistFile(asset.Path + ".meta"))
        {
            File.Delete(@"" + asset.Path + ".meta");
        }
    }
